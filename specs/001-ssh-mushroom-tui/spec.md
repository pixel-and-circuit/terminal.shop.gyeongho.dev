# Feature Specification: SSH-Based Mushroom Sales TUI

**Feature Branch**: `001-ssh-mushroom-tui`  
**Created**: 2025-02-11  
**Status**: Draft  
**Input**: User description: "mushroom.gyeongho.dev 는 ssh 기반 버섯 판매 플랫폼의 TUI 어플리케이션이다. 사용자는 ssh -a -i /dev/null mushroom.gyeongho.dev 명령어를 실행하여 버섯 판매 플랫폼의 TUI 어플리케이션에 접근할 수 있다. 사용자는 TUI에서 단축키로 shop(a) about(s) faq(d) 를 선택해서 각각 버섯 주문, ssh store에 대한 정보, faq 를 확인할 수 있다. 가장 중요한 것은, terminal.shop 프로젝트의 UI/UX를 그대로 따라하여 terminal.shop(커피)의 버섯 판매용 버전을 구현해야 한다는 점이다."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Access TUI via SSH and Navigate (Priority: P1)

A user runs `ssh -a -i /dev/null mushroom.gyeongho.dev` and is presented with a terminal user interface (TUI) that matches the look and feel of terminal.shop. The main screen offers clear navigation options: Shop (a), About (s), and FAQ (d). The user can use these single-key shortcuts to move between sections.
...
1. **Given** the user has SSH client access, **When** they run `ssh -a -i /dev/null mushroom.gyeongho.dev`, **Then** a TUI session starts and displays the main navigation (Shop, About, FAQ) with indicated shortcuts.
...
### User Story 2 - Shop: Browse and Order Mushrooms (Priority: P2)

A user selects Shop (a) from the main TUI. They can browse mushroom products, view details, and complete an order (e.g. add to cart and checkout) using the same interaction patterns as terminal.shop (coffee), adapted for mushroom products.

**Why this priority**: The primary business value is selling mushrooms; shop is the core transaction flow.
...
1. **Given** the user is on the main TUI, **When** they press (a) for Shop, **Then** the shop view opens and lists or allows browsing of mushroom products in a terminal.shop-style layout.
...
A user selects About (s) from the main TUI to learn about the SSH store and the mushroom farm. They see static content (e.g. story, contact, or branding) presented in the same visual and interaction style as terminal.shop.
...
- **FR-001**: System MUST allow access to the TUI via SSH using the command `ssh -a -i /dev/null mushroom.gyeongho.dev` (or equivalent host/command that reaches the same service).
...
- **FR-004**: System MUST allow users to open the Shop section and browse mushroom products and complete an order (add to cart and checkout) through the TUI.
...
- **Product (Mushroom)**: A sellable mushroom type or SKU; has name, description, and ordering-relevant attributes (e.g. price, availability).
...
- The hostname `mushroom.gyeongho.dev` (or the actual SSH host) is configured to accept the specified SSH command and start the TUI session.
...
- **SC-003**: A user familiar with terminal.shop can navigate the mushroom TUI (main menu, sections, back) without written instructions.
